...
/*   Updated: 2016/01/04 14:25:07 by gmarguer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_use_item(t_global *global, t_item *item, int y, int x)
{
	if (ft_valid_item(global, item, y, x))
		if (ft_write_item(global, item, y, x))
			return (1);
	return (ft_solve(global, yx(y, x, global), xy(x, global), item));
}
...
int		ft_resolve(t_global *global, int y, int x)
{
	...
	while (item)
	{
		...
		while (!(ft_solve(global, y, x, item)))
		{
